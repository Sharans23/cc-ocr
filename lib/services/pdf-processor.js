...
const OCR_SPACE_API_KEY = "K86418975288957"
  ...
  try {
    ...
    if (result.ParsedResults && result.ParsedResults.length > 0) {
      ...
      if (parsedResult.FileParseExitCode === 1) {
        console.log(`[v0] OCR successful! Extracted ${parsedResult.ParsedText?.length || 0} characters`)
        return parsedResult.ParsedText || ""
      } else {
        throw new Error(
          `OCR parsing failed: ${parsedResult.ErrorMessage || "Unknown error"} (Exit code: ${parsedResult.FileParseExitCode})`,
        )
      }
    }
...
export const processPDF = async (file, onProgress) => {
  ...
  console.log(`[v0] OCR complete! Total text length: ${fullText.length} characters`)
